...
## Get options
option_list <- list(
  make_option(c("--input", "-i"), action="store", default="C:/Users/ellie/Onedrive/Saez/Pipeline/github/CKD_QC/data/poorsample/", type='character',
...
indir <- paste(opt$input, "/outs/filtered_feature_bc_matrix", sep="") %>%
  str_replace_all("/outs/filtered_feature_bc_matrix/outs/filtered_feature_bc_matrix", "/outs/filtered_feature_bc_matrix") %>%
  str_replace_all("/outs/filtered_feature_bc_matrix//outs/filtered_feature_bc_matrix", "/outs/filtered_feature_bc_matrix") 
